...
namespace Luce.Implementation.Repositories
{
    public class CartItemRepository : GenericRepository<CartItem> , ICartItemRepository 
    {
        ...
        {
            var CartItem = await _Context.CartItems.Where(c => c.Id == cartItemId).Include(x => x.Product).Include(x => x.Customer).SingleOrDefaultAsync();
            return CartItem;
        }

        public async Task<IList<CartItem>> GetCartItemsAsync(int customerId)
        {
            var cartItems = await _Context.CartItems.Where(a => a.CustomerId == customerId).Include(x => x.Product).Include(x => x.Customer).ToListAsync();
            return cartItems;
        }
